...
but cannot resolve the SAFOD expected tidal velocity response — we quantify
...
| Pre-registered tidal null | amp 3.4×10⁻⁴ ± 2.3×10⁻⁴, p_null = 0.51, **95% UL = 7.98×10⁻⁴ (~14× the Niu SAFOD expected-response scale of 5.76×10⁻⁵)** | Best-executed piece; robust to dropping first 6 h |

**Benchmark change, 2026-08-10.** This row previously read "1.6× De Fazio". That framing was fragile and has been retired. De Fazio et al. (1973) is a single measurement on a Rayleigh-like mode guided by an air-filled mine shaft in Franklin marble at 500 Hz — not a prediction for a fluid-filled borehole. Its observed amplitude is also ambiguous in the available scan: the text says "of the order of 10⁻³" while Figure 3 supports ~5×10⁻⁴ peak-to-peak. Under the larger reading this upper limit would become 0.8×, i.e. *resolving* the benchmark, reversing the headline on a number that cannot be pinned down.

The **Niu SAFOD expected-response scale** replaces it: 2.4×10⁻⁷ Pa⁻¹ × 240 Pa = 5.76×10⁻⁵, both values verified verbatim in Niu et al. (2008). It is site-specific, physically motivated, and stable. Niu et al. independently concluded that tidal travel-time changes at SAFOD would be "predicted to be undetectable", so this null agrees with the only prior active-source velocity monitoring at the site. Cite De Fazio only as historical context — the largest reported guided-mode tidal response, ~5×10⁻⁴ to 10⁻³ — never as a threshold.
