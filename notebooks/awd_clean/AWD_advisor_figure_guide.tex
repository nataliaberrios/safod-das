...
\path{AWD_results_dashboard.ipynb}, version v50.  The hierarchical
...
\subsection{Seasonal robustness of the ambient F--K result (completed)}
...
\subsection{Independent unfiltered verification (completed)}
...
The first selected winter day (2024-12-20) has enough completed checkpoint products for a five-hour interim stack using 300 one-minute files. The exact-resolution unfiltered path gives a normalized-correlation score of 0.0003 at 3.2~km~s$^{-1}$, below its receiver-permutation 95th-percentile maximum (p = 0.265). The negative signed F--K branch peaks at 3.075~km~s$^{-1}$, has score 0.266 at 3.2~km~s$^{-1}$, and gives p = 0.0005; the legacy positive-mask curve was evaluated at the same positive lag and is superseded as a physical opposite-branch test, while the two-sided branch remains enhanced.
...
\subsection{v43 signed-lag audit and corrected status}
\figquestion{Were both causal and anti-causal correlation lags extracted and scored consistently?}

No. The legacy FFT correlation assembled its negative-lag half from a truncated padded array. The zero and positive lags were extracted correctly, so the reported negative-mask positive-lag moveout score and its receiver-permutation null remain reproducible. The negative-lag half was invalid. Consequently, the legacy positive-mask value $p=0.978$ tested leakage along the same positive-lag target moveout; it did not test whether a physical opposite-propagating branch exists at negative lag.

The corrected routine passes three deterministic regression tests: delayed and reversed impulses return opposite lags, and broadband waves traveling toward increasing and decreasing fiber coordinate are retained by opposite signed F--K masks and peak at opposite correlation lags. The corrected eight-day seasonal aggregation and independent completion audit subsequently finished successfully. Both physical lag branches peak at 3.075~km~s$^{-1}$ and have comparable magnitude after the frozen selection; the complete quantitative result and detailed caption are reported in the corrected eight-day section below.

\takeaway{Figure~39 remains valid only as a negative-mask positive-lag repeatability result. Figure~34's positive-mask curve is an opposite-mask leakage control, not a physical anti-causal branch test.}
...
The seasonal jobs processed eight selected dates independently and then formed an aggregate from 11,523 weighted one-minute files. All records were detrended, filtered from 5--20~Hz, divided by a 5-s running mean of absolute amplitude, and correlated with the channel-0 top-source geometry using the measured 1.020952-m spacing. The fixed negative signed wedge has a peak at 3.075~km~s$^{-1}$, score 0.279, and a 500-permutation receiver null probability $p=0.0002$. The legacy positive-mask score at positive lag is weak ($p=0.978$), but is now interpreted only as opposite-mask leakage along the negative branch's target moveout. It is not a test of the physical positive branch at negative lag. The two-sided positive-lag control is enhanced ($p=0.0026$), while the exact-resolution unfiltered aggregate is weak ($p=0.343$).
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{ambient_transfer/fk_seasonal_day_sections.png}
\caption{\textbf{Superseded legacy view of eight-day negative-mask positive-lag repeatability.} The eight date panels show independently stacked, channel-0 top-source correlations from complete selected days; the final panel is the across-day weighted aggregate. Every panel uses the same 5--20~Hz temporal normalization, measured 1.020952-m channel spacing, fixed negative signed F--K mask, lag and receiver-position limits, and display color scale. The dashed line is a 3.2~km~s$^{-1}$ reference moveout, not a fitted velocity. Similar receiver-position--lag structure across dates and its reinforcement in the aggregate provide the visual repeatability test. The aggregate negative branch reaches a peak near 3.075~km~s$^{-1}$ with score 0.279 and receiver-permutation $p=0.0002$ across 11,523 weighted files. Because the signed wedge includes 2.5--4.5~km~s$^{-1}$ by construction, this demonstrates repeatability of a selected transfer observable, not an independent formation-velocity estimate. The older summary scatter plot is retained only as supplementary QC.}\end{figure}
...
\subsection{Completed anti-alias preprocessing sensitivity}
\figquestion{Does the F--K result remain stable when the data are decimated only after explicit anti-alias filtering?}

The original comparison is withdrawn because it measured correlations before applying the signed F--K mask. The corrected workflow applies the identical negative signed 2.5--4.5~km~s$^{-1}$ F--K mask before correlation and receiver-permutation scoring to all three resampling paths. The completed five-hour test uses 300 one-minute files from 2024-12-20. Direct twofold slicing and explicit anti-aliased polyphase resampling both peak at 3.075~km~s$^{-1}$, whereas the undecimated full-resolution path peaks at 3.050~km~s$^{-1}$. Peak scores are 0.2790, 0.2772, and 0.2747, respectively; all three paths give $p=0.002$ against their receiver-order permutation nulls. The selected ridge is therefore not created by the tested factor-of-two decimation method, although this remains a validation of the selected F--K observable rather than independent evidence for its physical identity.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{ambient_transfer/alias_sensitivity_2024-12-20_start0_n300.png}
\caption{\textbf{Anti-alias and decimation sensitivity of the selected ambient F--K observable.} Columns compare direct factor-of-two slicing, explicit anti-aliased polyphase resampling by two along the time and channel axes, and processing at the original temporal and spatial sampling. Each path uses the same 300 one-minute records from 2024-12-20, 5--20~Hz preprocessing, 5-s running-absolute-mean temporal normalization, Nano channel~0 virtual-source geometry, negative signed 2.5--4.5~km~s$^{-1}$ F--K mask, physical positive-lag score, and 500 receiver-order permutations. The upper row shows correlation sections as a function of receiver separation and lag; the lower row shows median moveout score versus trial velocity, with the horizontal dashed line marking the path-specific 95th-percentile null and the vertical dotted line marking 3.2~km~s$^{-1}$. Direct and anti-aliased paths both peak at 3.075~km~s$^{-1}$, and full-resolution processing peaks at 3.050~km~s$^{-1}$; corresponding peak scores are 0.2790, 0.2772, and 0.2747, with $p=0.002$ for every path. The close agreement shows that the selected ridge is insensitive to the tested decimation implementation and is not a spatial- or temporal-alias artifact introduced by direct slicing. Because every path applies the same velocity wedge before scoring, this control validates preprocessing stability only; it does not independently identify the ridge as a P wave, establish Green's-function convergence, or provide an unbiased formation velocity.}\end{figure}

\takeaway{This legacy summary is retained for provenance but is superseded for branch physics and F--K-selection inference by the v44 ambient transfer validation below. The old positive-mask same-positive-lag result must not be interpreted as absence of the physical opposite branch.}
...
3.2~km~s$^{-1}$ are 0.270 and
0.267, with conditional receiver-order
probabilities $p=0.0005$ and
$p=0.0005$.  The observable therefore
...
3.200~km~s$^{-1}$ and raises
its score to 0.703.  The broad
2.0--5.5~km~s$^{-1}$ wedge instead peaks at
2.275 and
1.850~km~s$^{-1}$, and its
fixed 3.2~km~s$^{-1}$ probabilities are
$p=0.656$ and
$p=0.49$.  Direction-only scores
at 3.2~km~s$^{-1}$ fall to 0.0004 and
0.0107.  Thus the measured result is a
...
\subsection{Correction to the selection-conditioned full-pipeline statistic}
\figquestion{What did the v44 surrogate calculation actually test?}

The v44 probabilities remain reproducible, but their physical interpretation
was overextended. Channel permutations and independent circular shifts were
applied before F--K filtering, yet every observed and surrogate record was
then restricted to the same 2.5--4.5~km~s$^{-1}$ corridor and scored by a
velocity search inside that corridor. A velocity-wedge filter necessarily
imposes a compatible space--time covariance on its output. The statistic
therefore measures the response of the selected filter-plus-score operator to
coherence-disrupted inputs; it is not an independent test for a physical wave.

The earlier decision to withdraw the ridge is superseded. Equally, the large
probabilities do not confirm the ridge. F--K filtering remains a valid
directional wavefield operation; the selected ambient ridge is
\textbf{unresolved} pending a statistic measured before the mask or fixed
independently of the selected velocity support.
...
\caption{\textbf{Superseded 30-minute selected-operator diagnostic.} Observed branch-specific peak correlation statistics are compared with 20 channel-permutation and 20 independent-circular-shift realizations. Surrogates are generated before filtering, but every realization is subsequently restricted to the same 2.5--4.5~km~s$^{-1}$ F--K corridor and scored by a velocity search within it. The probabilities therefore quantify the response of the complete selected operator to coherence-disrupted inputs. Because selection and score share the same velocity support, the figure cannot reject F--K filtering or establish a propagating ambient-noise arrival. It is retained to document the superseded v44 decision path.}
\end{figure}
...
\caption{\textbf{Superseded five-hour selected-operator diagnostic.} The 300-file calculation repeats the pilot with unchanged Nano channel-0 geometry, 5--20~Hz preprocessing, temporal normalization, signed 2.5--4.5~km~s$^{-1}$ wedge, physical-lag convention, and maximum-over-velocity statistic. Surrogate selected scores are comparable to or greater than the observation. This demonstrates a strong corridor-conditioned filter-plus-score response; it does not demonstrate that F--K filtering is invalid or that a physical wave is absent.}
\end{figure}

\subsection{Independent pre-filter F--K energy validation (v45)}
\figquestion{Does the input wavefield contain excess ordered target-slowness
energy before any F--K mask is applied?}

For file $j$, let $P_j(f,k)$ be F--K power after detrending, 5--20~Hz
bandpass filtering, 5-s running-absolute-mean normalization, twofold temporal
and spatial decimation, and 10\% Tukey tapers, but before directional
selection. For branch $b$,
\[
S_b={1\over N}\sum_{j=1}^{N}\log\left[
{\operatorname{mean}_{(f,k)\in T_b}P_j(f,k)\over
 \operatorname{mean}_{(f,k)\in R_b}P_j(f,k)}\right],
\]
where $T_b$ is the predeclared 2.5--4.5~km~s$^{-1}$ target support and
$R_b$ is the broader 1.2--6.5~km~s$^{-1}$ signed reference support. The
statistic is never evaluated on an F--K-filtered correlation section.

Channel permutation retains individual trace spectra while destroying receiver
order. Independent circular time shifts retain each trace spectrum while
disrupting interchannel phase alignment. The familywise statistic is the
maximum of the two signed branches, and the predeclared criterion requires
upper-tail $p\leq0.05$ for \emph{both} null families. Synthetic controls
pass: a 3.2~km~s$^{-1}$ plane wave enriches the correct branch, an
off-target 1.8~km~s$^{-1}$ wave is rejected, and noise is not forced into
the target interval.

The completed 300-file test gives geometric target/reference ratios 0.95229
for $F K<0$ and 0.95227 for $F K>0$. Both channel-permutation and
independent-time-shift familywise probabilities are $p=1.000$. The raw
normalized wavefield therefore has no excess target-corridor power under this
statistic. This does not invalidate F--K filtering; it means the selected
correlation ridge is not independently corroborated as a physical ambient
arrival by the current pre-filter and unfiltered controls.

\IfFileExists{ambient_transfer/fk_prefilter_energy_v1_n300_r20/fk_prefilter_energy_v1_aggregate_figure.png}{
\begin{figure}[H]
\centering
\includegraphics[width=0.99\linewidth]{ambient_transfer/fk_prefilter_energy_v1_n300_r20/fk_prefilter_energy_v1_aggregate_figure.png}
\caption{\textbf{Independent pre-filter F--K energy validation.} The upper-left panel shows mean 5--20~Hz F--K power before any velocity mask or channel-pair correlation; white curves delimit the predeclared target corridors for both signs of $F K$. The upper-right and lower-left panels compare observed mean log target/reference enrichment with channel-permutation and independent-time-shift null ensembles; colored lines are observed branch values and the dotted line is the familywise 95th percentile. The lower-right panel shows geometric target/reference ratios, for which unity denotes no enrichment relative to the broader signed support. The observed geometric ratios are 0.95229 and 0.95227 and both familywise probabilities are $p=1.000$, so the predeclared $p\leq0.05$ requirement is not met. The result supplies no independent target-power enrichment and does not support a physical ambient arrival under this statistic. It does not invalidate F--K filtering, prove absence of weak coherent energy, identify wave type, or establish formation velocity.}
\end{figure}
}{
\noindent\textit{The completed aggregate product is missing from this copy.
Re-run the versioned aggregation command; do not substitute the 10-file pilot.}
}

\subsection{Independent and secondary controls}

The exact-resolution unfiltered aggregate contains 11,523 weighted one-minute
files. On the fixed 3.2~km~s$^{-1}$ trajectory, the causal score is 0.002711
with permutation $p=0.341$ and the anti-causal score is 0.0000929 with
$p=0.202$. These tests do not independently confirm the ridge, but they do
not test whether F--K filtering is legitimate.

Subtracting the mean surrogate correlation section as an estimated filter
kernel gives inconsistent secondary results. The channel-permutation residual
peaks near the upper search edge at 4.45~km~s$^{-1}$, only 0.05~km~s$^{-1}$ below the 4.5~km~s$^{-1}$ limit, with $p=0.0476$,
whereas the independent-time-shift residual gives $p=0.952$ or larger and
the 3.2~km~s$^{-1}$ residual score is negative. Disagreement between null
families and the upper-edge peak preclude promotion as physical evidence.

\takeaway{F--K filtering is valid, and the selected ambient observable repeats
across held-out dates. Its physical interpretation remains unresolved: the
v44 rejection was circular, the strict unfiltered control is weak, and the
independent pre-filter test finds no target-power enrichment. No Green's-function, mode, or
formation-$V_P$ claim is assigned until the validation is complete.}

\subsection{Velocity-fan-conditioned signed-lag symmetry diagnostic (corrected v49)}
...
correlation is 0.9959, relative RMS difference is
0.091, and norm ratio is
1.007.  This is a property
...
\midrule
1.8 & increasing & not reached & not reached \\
1.8 & decreasing & not reached & not reached \\
2.75 & increasing & 0.03 & 0.03 \\
2.75 & decreasing & 0.03 & 0.03 \\
3.2 & increasing & 0.03 & 0.003 \\
3.2 & decreasing & 0.03 & 0.003 \\
4 & increasing & 0.03 & 0.1 \\
4 & decreasing & 0.1 & 0.1 \\
\bottomrule
\end{tabular}
...
a reflected-branch correlation of 0.995, whereas standard
direction-only selection gives 0.264.  Direction-only scores
at 3.2~km~s\( ^{-1} \) are -0.0004 and
-0.0107.  Before masking, the ratio of total positive- to
negative-wavenumber power is 1.0039, and the frozen target-corridor
...
0.997, and the filtered score ratio is
1.008; the recorded low-frequency
...
\caption{\textbf{Validation ladder for the signed SAFOD ambient F--K operator.} (a) Exact 10-Hz, 3.2-km-s\( ^{-1} \) plane waves are propagated separately toward increasing and decreasing fiber coordinate, decimated identically to the production workflow, and passed through complementary 5--20-Hz, 2.5--4.5-km-s\( ^{-1} \) signed masks. Correct/wrong output-RMS ratios are 9.86:1 and 9.95:1, validating Fourier sign. (b) Minimum injected broadband RMS relative to median real-channel 5--20-Hz RMS required for independent pre-filter recovery after injection into 300 real one-minute records. Both directions are recovered for all in-wedge test speeds; the omitted 1.8-km-s\( ^{-1} \) off-wedge control is never recovered. (c) Full-resolution application to the known surface-AWD Nano stack over 80--440~m. Energy within a fixed \(\pm12\)-ms tube around the independently estimated 2.975-km-s\( ^{-1} \) increasing-coordinate moveout gives expected/opposite ratios 1.18 at 5--20~Hz and 139.3 at 25--60~Hz, demonstrating frequency-dependent empirical directionality. (d) The unmasked ambient target-fan power ratio is 0.997, and the independently filtered 3.2-km-s\( ^{-1} \) score ratio is 1.008. Thus the two branches are valid coordinate-direction outputs rather than a software mirror, while the physical origin of the decreasing-coordinate energy remains unresolved. The figure does not establish two body-wave Green's-function branches, wave type, or an independent formation velocity.}
\end{figure}
...
One complete UTC day (1,440 one-minute files;
5,759 overlapping 30-s windows) is processed with
...
\begin{figure}[H]
\centering
\includegraphics[width=0.99\linewidth]{ambient_transfer/lellouch2019_reproduction_v1/lellouch2019_2024-12-20_start0_requestedall_used1440.png}
\caption{\textbf{Transfer of the Lellouch et al. (2019) ambient-interferometry workflow to the 2024--2025 SAFOD Nano acquisition.} A complete UTC day (1,440 contiguous one-minute files) is divided into 5,759 30-s windows with 15-s overlap. Nano channel~0 is the fixed top virtual source; target receiver positions increase down the cemented main-hole fiber at 50-m intervals from 50 to 800~m using measured 1.020952-m spacing. Phase/strain is differentiated to a strain-rate proxy and divided by a 5-s running absolute mean before correlation; the daily stack is filtered at 5--20~Hz. Black variable-area wiggles show positive correlation lag, and the red line is a fixed 3.2-km-s\( ^{-1} \) reference rather than a fit. (a) Individual receiver correlations. (b) The published local stack over receiver channels \(R-10\) through \(R+10\), retaining the same channel-0 source. (c) The same 21 correlations shifted at fixed 3.2-km-s\( ^{-1} \) travel-time differences before stacking. Fixed-trajectory causal scores are 0.0048, 0.0070, and 0.0066; the aligned absolute score is 1.36 times the isolated value. The aligned descriptive causal maximum is 2.025~km~s\( ^{-1} \), with causal/anti-causal absolute-score ratio 0.74 at 3.2~km~s\( ^{-1} \). This is a documented transfer, not a bit-for-bit reproduction of the 2017 records; acquisition epoch, interrogator/gauge settings, ambient source distribution, and channel-origin inheritance can affect the comparison.}
\end{figure}
