...
\path{AWD_results_dashboard.ipynb}, version v48.  The hierarchical
...
\subsection{Corrected eight-day signed-lag comparison (v47)}
...
The independently audited aggregate contains 11,523 one-minute files from
8 frozen dates. \(F K<0\) is evaluated at positive lag for propagation
...
The negative branch peaks at 3.075~km~s\( ^{-1} \),
has absolute score 0.2710 at 3.2~km~s\( ^{-1} \),
and receiver-permutation \(p=0.0002\). The corrected positive
branch peaks at 3.075~km~s\( ^{-1} \), has
absolute score 0.2688, and
\(p=0.0002\). Their positive/negative absolute-score ratio is
0.992, with normalized asymmetry 0.004; the branches are
comparable in magnitude after signed F--K selection.

\begin{figure}[H]
\centering
\includegraphics[width=0.99\linewidth]{ambient_transfer/signed_lag_v2/seasonal_signed_fk_v2_aggregate.png}
\caption{\textbf{Corrected eight-day signed-lag comparison after independent completion audit.} The aggregate uses 11,523 one-minute records from 8 preselected dates, Nano channel~0 as the virtual source, increasing offset along the cemented fiber, 5--20~Hz filtering, 5-s running-absolute-mean normalization, and a frozen 2.5--4.5~km~s\( ^{-1} \) signed F--K wedge. The upper-left panel shows \(F K<0\) on positive lag; the upper-right shows \(F K>0\) on negative lag, with a common correlation scale. Black dashed trajectories mark each branch's conditional score peak at 3.075~km~s\( ^{-1} \); gray dotted trajectories mark the fixed 3.2~km~s\( ^{-1} \) comparison. The lower-left shows moveout scores and receiver-permutation 95\% thresholds; the lower-right compares physical and opposite-lag leakage at 3.2~km~s\( ^{-1} \). Branches peak at 3.075 and 3.075~km~s\( ^{-1} \), with \(p=0.0002\) and \(p=0.0002\); their score ratio is 0.992. The audit reproduces scores, verifies signs, and confirms all files. Significance is conditional on F--K selection and does not independently establish Green's-function convergence, causality, propagation identity, wave type, or formation velocity.}
\end{figure}

\takeaway{Both physical lag sides have now been computed correctly and are
comparable in magnitude. Their significance remains conditional on F--K selection. The v45
