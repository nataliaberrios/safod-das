...
\path{AWD_results_dashboard.ipynb}, version v12.  The hierarchical
...
review sit above the full diagnostic record.  The all-shot PGSI timing and
moveout calibration remains explicitly preliminary, and the Deep
tube-wave label remains a physical interpretation rather than a demonstrated
...
The previously outstanding repeatability and Deep validation branches have been run and audited.  Version v10 added a bounded Deep coordinate-registration and tube-wave consistency test; v11 added a source-history regression and phase-free tidal-timescale stress test; v12 adds exploratory CCA with serial-dependence-aware nulls and blocked validation.  These results are deliberately separated from the remaining physical interpretation:
...
The authoritative dashboard was promoted to v12 after the corrected hierarchical
...
source-history/tidal-timescale regression, exploratory CCA, and the six-figure core presentation set were
...
response still uses proxy-based source correction.  Its useful role is
prioritization: obtain a physically phased Earth-tide strain/traction predictor
and rerun CCA with environmental controls and held-out bursts.  The current
canonical variate is not a fault-opening measurement.
...
\takeaway{\preliminary: CCA is useful for prioritizing a physically phased tidal predictor, but its serial-dependence-aware null and blocked validation do not confirm the candidate delay pattern.}
...
Fault opening/closing interpretation & Not yet tested & Requires a physically phased Earth-tide strain/traction predictor and sign convention \\
CCA tidal-timescale association & Exploratory / not confirmed & Canonical $r=0.533$; row $p=0.019$; 3-burst block $p=0.118$; held-out correlations unstable \\
...
\begin{enumerate}[leftmargin=*]
  ...
  inversion.
  \item \textbf{Physically phased tidal test.} Replace the phase-free 24-hour
  harmonic with astronomical Earth-tide strain or fault-normal traction, add
  environmental controls, and repeat with held-out bursts before assigning the
  extrema to opening or closing.
  \item \textbf{CCA with physical forcing.} Replace the phase-free temporal basis
  with astronomical Earth-tide strain or fault-normal traction, include environmental
  controls, and rerun CCA with held-out bursts before interpreting canonical phases.
...
current bounded forward test supports tube-wave-like kinematics only.  The source-history delay regression adds a candidate nonmonotone 24-hour component, but its phase is not yet tied to a physical tidal traction.  Exploratory CCA prioritizes the same candidate but does not confirm it under serially aware nulls.  A formation-$V_P$ or
