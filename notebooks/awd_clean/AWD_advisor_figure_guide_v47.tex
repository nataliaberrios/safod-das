...
\path{AWD_results_dashboard.ipynb}, version v47.  The hierarchical
...
\subsection{Anti-alias preprocessing sensitivity (validation rerun in progress)}
\figquestion{Does the F--K result remain stable when the data are decimated only after explicit anti-alias filtering?}

The first saved anti-alias comparison is withdrawn as an F--K validation. An implementation error compared direct, anti-aliased, and full-resolution correlations before applying the signed F--K mask, so its numerical differences were unfiltered preprocessing differences rather than the intended test. The script has now been corrected so every resampling path is filtered with the same negative signed F--K mask before correlation and receiver-permutation scoring. A 30-minute pilot and a five-hour comparison have been resubmitted; this subsection will be replaced with their results after completion.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{ambient_transfer/alias_sensitivity_2024-12-20_start0_n30.png}
\caption{\textbf{Withdrawn anti-alias pilot figure.} This provenance figure compares direct, anti-aliased, and full-resolution paths, but the original implementation measured correlations before applying the signed F--K mask. It is therefore not a valid F--K preprocessing test and should not be used for interpretation; corrected results are pending.}\end{figure}
