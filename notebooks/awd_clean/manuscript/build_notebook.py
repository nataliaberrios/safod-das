...
md(
 ...
 "velocity changes — 1% on the cemented fibre, 0.5% on the wireline outbound",
 ...
 "| Cut 3.5 s windows from raw fibre, stack into bursts | no — load the stacks `.npz` | `paired_stack_job_deep_all.py` |",
 "| Inventory QC and the 49 → 46 burst question | **yes** | — |",
 "| Band-pass and channel selection | **yes** | — |",
 "| Record sections for both fibres | **yes** | — |",
 ...
 "",
 "The two 'no' rows at the top read raw fibre data off `$OAK` and take about",
...
md("---", "", "## 2. The data, and the burst inventory",
   "",
   "The stacks file holds one waveform per burst per channel, for both fibres,",
...
code(
 ...
 "counts    = np.asarray(meta['n_common'], dtype=int)   # drops per burst, both fibres",
 ...
 "      f'  -> {meta[\"nano_stacks\"].shape[1]*dx_nano:.0f} m of fibre')",
 "print(f'Deep  {meta[\"deep_stacks\"].shape[1]:5d} channels at {dx_deep:.4f} m'",
 "      f'  -> {meta[\"deep_stacks\"].shape[1]*dx_deep:.0f} m of fibre')",
 "print(f'slots {counts.size}, of which {(counts>0).sum()} have drops on both fibres')",
...
md("### Why 46 bursts and not 49",
   ...
   "them have no drops recorded on **both** fibres. They are not scattered",
   "dropouts and nothing was cut for quality — they are the last three bursts,",
   "because the Deep fibre stopped recording before the survey ended.")
...
code(
 ...
 "ok = counts > 0",
 "ax.bar(hours[ok],  counts[ok],  width=0.32, color='#2a78d6', label='drops on both fibres')",
...
code(
 ...
 "print(f'used by the stacks (both fibres)  {counts.sum()}   <- the analysed dataset')",
 "print()",
...
md("---", "", "## 3. Preprocessing — what has already been applied",
   ...
   "3. **The two fibres are in different units.** Nano is strain *rate*, Deep is",
   ...
   "   comparison. It would affect any amplitude comparison between fibres, and",
   "   none is made.",
   "",
...
# --------------------------------------------------------------------------
md("---", "", "## 4. What each fibre records",
   "",
   "Average the bursts into one section per fibre and look at them. This is the",
...
code(
 ...
 "    ax.set(xlabel='time from drop (s)', ylabel='distance along fibre (m)', title=title)",
 "",
...
md("---", "", "## 5. Finding the arrival: the semblance scan",
   "",
   "An arrival travelling along the fibre is a straight line in distance–time, so",
...
code(
 ...
 "    a.set(xlabel='lag (s)', ylabel='distance along fibre (m)', title=name)",
 "",
 ...
 "ax[2].invert_yaxis()",
 "ax[2].set(xlabel='lag (s)', ylabel='distance along fibre (m)', title='deconvolution, wiggles')",
 "plt.show()",
)
...
md("---", "", "## 7. The estimator: turning delays into a velocity change",
   ...
   "usable signal-to-noise 3 km down the fibre. Compare one burst's beam against",
...
md("---", "", "## 9. What was checked, and what this does not show",
   ...
   "",
   "- The measured quantity is a fractional change in the **apparent along-fibre",
   "  speed of a guided mode**. Not formation V_P or V_S, not stress, pore",
   "  pressure, permeability, or strain.",
   "- **No depth resolution.** Position is distance along fibre; the depth mapping",
