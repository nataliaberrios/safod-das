...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    """Replace one guarded text block and fail rather than silently drift."""
    if text.count(old) != 1:
        raise ValueError(
            f"expected one {label} block, found {text.count(old)}"
        )
...
cannot supply an unbiased velocity estimate or establish wave type. On Nano,
the increasing-coordinate branch is nominally downgoing and the
decreasing-coordinate branch is nominally upgoing under the known same-fiber
orientation. What remains unresolved is branch purity and physical origin:
the nominal upgoing energy may include reflections, scattering, or F–K leakage.
"""
    ...
    seasonal_status = unique_cell(
        notebook, "## Seasonal robustness of the ambient F–K result"
    )
    seasonal_text = "".join(seasonal_status["source"])
    seasonal_text = replace_once(
        seasonal_text,
        "The analysis is still running when this notebook section is first opened. "
        "The status cell below is therefore an intentional live checkpoint rather "
        "than a scientific conclusion.\n",
        "**Status update v50 — completed.** All eight preselected dates were "
        "processed, yielding 11,523 weighted one-minute records. Both signed "
        "branches peak at 3.075 km/s and pass the receiver-order permutation "
        "test (p = 0.0002 for each). This establishes seasonal repeatability of "
        "the fan-selected observable; it does not independently estimate "
        "velocity or assign physical upgoing/downgoing identity.\n",
        "notebook seasonal-running status",
    )
    seasonal_status["source"] = lines(seasonal_text)

    seasonal_caption_cell = unique_cell(
        notebook, "**Figure caption (when the completed comparison is available).**"
    )
    seasonal_caption = "".join(seasonal_caption_cell["source"])
    seasonal_caption = seasonal_caption.replace(
        "**Figure caption (when the completed comparison is available).**",
        "**Historical caption — completed products are reported in v41 and "
        "the v50 status above.**",
        1,
    )
    seasonal_caption = seasonal_caption.replace(
        "The across-day aggregate is computed only after the individual-day "
        "products are available; null thresholds are generated by "
        "receiver-separation permutations within each completed day. Seasonal "
        "differences would indicate that the apparent mode depends on environmental "
        "or coupling conditions, whereas consistent velocity and branch asymmetry "
        "would support a persistent propagating wavefield.",
        "The completed across-day aggregate and receiver-separation permutation "
        "nulls are reported later in the notebook. They demonstrate persistence "
        "of the selected fan response across the sampled dates, while leaving "
        "velocity and physical wave type conditional on independent evidence.",
        1,
    )
    seasonal_caption_cell["source"] = lines(seasonal_caption)

    unfiltered_status = unique_cell(
        notebook, "## Independent unfiltered verification"
    )
    unfiltered_text = "".join(unfiltered_status["source"])
    unfiltered_text = replace_once(
        unfiltered_text,
        "The comparison is not complete until the independent unfiltered "
        "day-level products and permutation nulls exist.\n",
        "**Status update v50 — completed.** The exact-resolution aggregate "
        "contains 11,523 weighted one-minute records. Its fixed 3.2 km/s "
        "causal score is 0.002711 (receiver-order p = 0.341), confirming that "
        "the broad unfiltered statistic remains weak. This control does not "
        "diagnose a filtering bug and does not invalidate a standard F–K "
        "passband used to isolate low-SNR coherent energy.\n",
        "notebook unfiltered-incomplete status",
    )
    unfiltered_status["source"] = lines(unfiltered_text)

    directional_caption_cell = unique_cell(
        notebook, "**Publication-style caption. Standard directional"
    )
    directional_caption = "".join(directional_caption_cell["source"])
    directional_caption = replace_once(
        directional_caption,
        ...
        "The audit therefore supports the correctness of the F–K implementation. "
        "It shows that the narrow-fan ridge is a selected, operator-conditioned "
        "observable rather than an unbiased velocity estimate; failure of the "
        "much broader direction-only statistic is a sensitivity result and does "
        "not reject the standard fan filter.\n",
        "notebook directional-audit conclusion",
    )
    directional_caption_cell["source"] = lines(directional_caption)
...
branches. For Nano, use **nominal downgoing / increasing-coordinate (F×K<0)**
and **nominal upgoing / decreasing-coordinate (F×K>0)**. These labels follow
the known top-to-bottom fiber orientation and the validated Fourier sign. They
do not imply perfect separation or prove that the nominal upgoing energy is a
single reflected body-wave branch.
...
are not a software mirror. On the downward-increasing Nano coordinate they are
nominal downgoing and upgoing outputs. The tests do not determine whether the
nominal upgoing energy is a discrete reflection, scattering, illumination from
below, or leakage where wavefields overlap near zero wavenumber; nor do they
provide an independent formation-velocity estimate.
"""
...
type; v50 supplies the completed validation ladder.}"""
    ...
    stale_directional_caption_end = r"""The audit supports the correctness of the F--K implementation but classifies the earlier narrow-fan ambient ridge as operator-conditioned rather than as an independently demonstrated physical wave.}"""
    corrected_directional_caption_end = r"""The audit supports the correctness of the F--K implementation. It shows that the narrow-fan ridge is a selected, operator-conditioned observable rather than an unbiased velocity estimate; failure of the broader direction-only statistic is a sensitivity result and does not reject the standard fan filter.}"""
    tex = replace_once(
        tex,
        stale_directional_caption_end,
        corrected_directional_caption_end,
        "TeX directional-audit caption conclusion",
    )
...
\takeaway{{For Nano, whose coordinate increases downhole, \(F K<0\) is the
nominal downgoing/increasing-coordinate branch and \(F K>0\) is the nominal
upgoing/decreasing-coordinate branch.  This coordinate/sign calibration does
not establish perfect branch purity or identify the origin of the nominal
upgoing energy.  The standard fan is valid as an independently motivated
passband, but cannot alone estimate velocity within that passband.}}
