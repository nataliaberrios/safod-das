#!/usr/bin/env python3
"""Resumable one-day driver for the frozen ambient F-K mask audit."""
...
def main():
    ...
    log = OUT / f"fk_mask_sensitivity_v2_{date}.log"
    with log.open("a") as handle:
        handle.write(
            f"RUN date={date} role={'development' if date == '2024-12-20' else 'held_out'} "
            f"files={nfiles} chunk={CHUNK}\n"
        )
        handle.flush()
        for start in range(0, nfiles, CHUNK):
            ...
            command = [
                sys.executable,
                str(ROOT / "ambient_fk_mask_sensitivity_v2.py"),
                "--date", date,
                "--start", str(start),
                "--nfiles", str(count),
                "--output-dir", str(OUT),
            ...
            result = subprocess.run(
                command, cwd=ROOT.parent, capture_output=True, text=True
            )
            handle.write(result.stdout)
            handle.write(result.stderr)
            handle.write(
                f"END start={start} rc={result.returncode} "
                f"elapsed={time.time() - began:.1f}\n"
            )
            handle.flush()
            if result.returncode != 0:
                raise SystemExit(
                    f"mask chunk failed: {date} start={start} rc={result.returncode}"
                )
