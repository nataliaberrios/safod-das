"""
Lellouch et al. 2019 Figure 7c reproduction — 2017 SEG-Y data, with common-mode removal.

Identical to sanity_cc_segy.py except one extra step is applied per window
before bandpass + temporal normalization:

  Common-mode noise removal: subtract the median trace across a reference
  channel range (default: all good channels) at each time sample.  This
  kills spatially-coherent noise (pump tones, surface vibrations) that
  survives temporal normalization because it is narrow-band rather than
  impulsive.  The OptaSense QC document for this dataset specifies channel
  range 200-900 for the reference estimate.

Additional environment variables (on top of sanity_cc_segy.py):
  SANITY_CMN_CH_LO : lower bound of reference channels for median (default 200)
  SANITY_CMN_CH_HI : upper bound exclusive (default 900)
"""
...
# Common-mode noise reference channel range (channel indices, not absolute)
CMN_CH_LO = int(os.environ.get("SANITY_CMN_CH_LO", "200"))
CMN_CH_HI = int(os.environ.get("SANITY_CMN_CH_HI", "900"))

# Lellouch-faithful CC parameters (identical to sanity_cc_segy.py)
FMIN, FMAX   = 5.0, 20.0
WINDOW_SEC   = 30.0
OVERLAP      = 0.5
MAX_LAG      = 1.0
...
def common_mode_removal(Xw, bad_mask, nch):
    """Subtract median trace over reference channel range from every channel.

    Reference range is [CMN_CH_LO, CMN_CH_HI) clipped to [0, nch).
    Bad channels are excluded from the median estimate.
    """
    lo = max(0, CMN_CH_LO)
    hi = min(nch, CMN_CH_HI)
    if lo >= hi:
        return Xw
    ref_mask = np.zeros(nch, dtype=bool)
    ref_mask[lo:hi] = True
    ref_mask[bad_mask] = False  # exclude bad channels from reference
    if ref_mask.sum() == 0:
        return Xw
    common = np.median(Xw[ref_mask, :], axis=0)  # (nsamples,)
    Xw = Xw - common[None, :]
    return Xw
...
def main():
    ...
    print("=== SAFOD DAS Lellouch CC — 2017 SEG-Y + common-mode removal ===")
    ...
    print(f"  CMN ref chs : [{CMN_CH_LO}, {CMN_CH_HI})")
    ...
    for _, row in tqdm.tqdm(sel.iterrows(), total=len(sel)):
        try:
            ...
            for i0 in range(0, npts - win_npts + 1, step_npts):
                ...
                # Common-mode noise removal (not in Lellouch stated params,
                # but consistent with OptaSense QC preprocessing for this dataset)
                Xw = common_mode_removal(Xw, bad_mask, actual_nch)
    ...
    out_name = (f"sanity_cc_segy_cmn_{date_tag}_hours{HOUR_MODE}"
    ...
    np.savez(
        ...
        cmn_ch_lo=CMN_CH_LO, cmn_ch_hi=CMN_CH_HI,
