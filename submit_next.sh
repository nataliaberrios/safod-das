...
echo "OUTPUT_VERSION=$OUTPUT_VERSION USE_TEMPORAL_NORMALIZATION=$USE_TEMPORAL_NORMALIZATION USE_SECOND_BANDPASS=$USE_SECOND_BANDPASS TN_WINDOW=$TN_WINDOW"
sbatch --export=ALL,OUTPUT_VERSION="$OUTPUT_VERSION",USE_TEMPORAL_NORMALIZATION="$USE_TEMPORAL_NORMALIZATION",USE_SECOND_BANDPASS="$USE_SECOND_BANDPASS",TN_WINDOW="$TN_WINDOW" --array="$NEXT_INDICES" run_daily_array.sbatch
